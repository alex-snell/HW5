 ...
 class BloomFilter {
     ...
     public boolean contains(String s) {
        //check
         ...
         //noHashes = numberofHashes used
         for(int n = 0; n < noHashes; n++){
            //create hashCode for s at first hash
            long hcode = hashCode(s, n);
            //get bit position
            int bitPosition = (int) (hcode) & this.hashMask;
            //if binary data is not at position return false
            if(!data.get(bitPosition)){
                return false;
            }
         }
         return true;
