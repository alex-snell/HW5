 ...
 public class CuckooHash<K, V> {
      ...
      */
      ...
      public void put(K key, V value) {
        ...
        // If the same key-value pair already exists, return
        ...
        // Try to find an empty spot first
        if (table[pos1] == null) {
        ...
        // Both positions are occupied, start the displacement process
        K currentKey = key;
        V currentValue = value;
        int count = 0;
        int currentPos = pos1; // Start with the first position
    
        while(count < CAPACITY) {
            //get displaced elements
            K displacedKey = table[currentPos].getBucKey();
            V displacedValue = table[currentPos].getValue();
            
            //store new value
            table[currentPos] = new Bucket<K, V>(currentKey, currentValue);
            
            //update variables
            currentKey = displacedKey;
            currentValue = displacedValue;
            
            // find the alternate position for the displaced element
        ...
        // If we reach here, we've detected a cycle
        // Need to rehash and then insert the current element that couldn't find a place
        rehash();
